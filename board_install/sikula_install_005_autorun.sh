...
tmux new-session -s "\$tmux_session" -d \$autorum_dir/panel_left.sh
...
cat << EOF > /root/autorun/panel_left.sh
#!/bin/bash
echo \$0
/bin/bash # Start interactive shell, otherwise the panel closes
EOF

chmod +x /root/autorun/panel_left.sh

cp /root/autorun/panel_left.sh /root/autorun/panel_topright.sh
cp /root/autorun/panel_left.sh /root/autorun/panel_bottomright.sh
